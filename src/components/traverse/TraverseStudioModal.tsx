...
export const TraverseStudioModal: React.FC<TraverseStudioModalProps> = ({
  ...
  const initialBearingDec = useMemo(() => {
    return dmsToDecimal(initBearingDeg || 0, initBearingMin || 0, initBearingSec || 0);
  }, [initBearingDeg, initBearingMin, initBearingSec]);
  ...
  const result = useMemo(() => {
    try {
      ...
      const sControl: CoordinatePoint = {
        id: startPointId || 'CP_START',
        easting: typeof startEasting === 'number' && !isNaN(startEasting) ? startEasting : BENCHMARK_START_CONTROL.easting,
        northing: typeof startNorthing === 'number' && !isNaN(startNorthing) ? startNorthing : BENCHMARK_START_CONTROL.northing,
        isControl: true
      };

      const cControl: CoordinatePoint = isClosedLoop
        ? sControl
        : {
            id: 'CP_CLOSE',
            easting: typeof closeEasting === 'number' && !isNaN(closeEasting) ? closeEasting : BENCHMARK_START_CONTROL.easting,
            northing: typeof closeNorthing === 'number' && !isNaN(closeNorthing) ? closeNorthing : BENCHMARK_START_CONTROL.northing,
            isControl: true
          };

      return computeTraverseAdjustment(sControl, cControl, initialBearingDec, stations, method, order);
    } catch (err) {
      console.error('Traverse calculation error:', err);
      return null;
    }
  }, [startPointId, startEasting, startNorthing, isClosedLoop, closeEasting, closeNorthing, initialBearingDec, stations, method, order]);
  ...
  const handleLoadBenchmark = () => {
    ...
    setStartPointId(BENCHMARK_START_CONTROL.id);
  ...
  const handleInjectIntoCAD = () => {
    if (!result || !result.balancedStations || result.balancedStations.length < 3) {
  ...
  return (
    <ErrorBoundary fallbackTitle="Traverse Studio Recovery">
              ...
              <div className="traverse-reduction-view">
                {result ? (
                    ...
                    <div className="precision-card-grid">
                      ...
                      <div className="precision-card">
                        ...
                        <div className="precision-val text-cyan">
                          {(result.totalLinearMisclose || 0).toFixed(3)} m
                        </div>
                        <div className="precision-sub">
                          ΔE = {(result.linearMiscloseE || 0).toFixed(3)}m | ΔN = {(result.linearMiscloseN || 0).toFixed(3)}m
                        </div>
                      </div>
                      ...
                      <div className="precision-card">
                        ...
                        <div className="precision-val">
                          {(result.totalPerimeter || 0).toFixed(2)} m
                        </div>
                        <div className="precision-sub">
                          {result.legs?.length || 0} Balanced Legs ({result.adjustmentMethod})
                        </div>
                      </div>
                    </div>
                        ...
                        <tbody>
                          {result.legs?.map((leg, idx) => (
                            <tr key={idx}>
                              ...
                              <td className="mono-cell">{(leg.distance || 0).toFixed(3)}</td>
                              <td className="mono-cell">{(leg.rawDeltaEasting || 0).toFixed(3)}</td>
                              <td className="mono-cell">{(leg.rawDeltaNorthing || 0).toFixed(3)}</td>
                              <td className="mono-cell" style={{ color: '#f59e0b' }}>{(leg.correctionEasting || 0).toFixed(3)}</td>
                              <td className="mono-cell" style={{ color: '#f59e0b' }}>{(leg.correctionNorthing || 0).toFixed(3)}</td>
                              <td className="mono-cell text-cyan" style={{ fontWeight: 600 }}>{(leg.balancedEasting || 0).toFixed(3)}</td>
                              <td className="mono-cell text-cyan" style={{ fontWeight: 600 }}>{(leg.balancedNorthing || 0).toFixed(3)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="empty-state-card" style={{ padding: '30px', textAlign: 'center' }}>
                    <p style={{ color: 'var(--text-muted)' }}>Enter at least 3 valid stations in the Field Book tab to calculate traverse balancing.</p>
                  </div>
                )}
              </div>
            )}
